	...
	public static void main(String[] args) {
		...
		for( ; ; ) {
			...
			if(menu == 1) {
				...
				Student temp = new Student(grade, ban, bunho, name);
				...
				// 입력한 학생 정보가 없으면 끝
				// 학생 정보들 중에서 하나씩 비교하여 학년, 반, 번호가 같은 학생이 있는지 확인해서 없으면 조건문 종료
				
				int index = -1;
				
				Student temp = new Student(grade, ban, bunho);
				...
				boolean res = std[index].addScore(new Score(sub, dict, middle, fin, perfor));
